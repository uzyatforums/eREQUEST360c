...
| **5** | **Standard Row Actions** | Every configuration master grid must feature standard row actions: View (👁), Edit (✏), Copy (📄), and Activate/Deactivate (🔄). Copy shall pre-populate the Create form without duplicating PK, audit, or unique business code. | `Eye` (View), `Edit2` (Edit), `Copy` (Pre-populate Create), and `StatusBadge`/`Dialog` (Activate/Deactivate) integrated. | [x] Passed |
...
| **16** | **No-Delete Configuration Lifecycle** | Configuration master records must not provide a delete UI action. Records are retired via deactivation to preserve auditability, integrity, and banking compliance. | Delete UI controls prohibited; retirement managed strictly by deactivation. | [x] Passed |
