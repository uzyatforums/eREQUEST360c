...
async function runBrowserTests() {
  ...
  try {
    ...
    results.push({
      criterion: '1. Default Route after login',
    ...
    await page.click('tr.group');
    await page.waitForTimeout(500);
    const url2 = page.url();
    const body2 = await page.innerText('body');
    const pass2 = url2.startsWith('http://localhost:5173/card-programmes/') && body2.includes('Child Entity Management Workspaces');
    await page.screenshot({ path: path.join(screenshotDir, '02_details.png') });
    results.push({
      criterion: '2. Details Page Navigation',
    ...
    await page.click('text="Charges & Posting"');
    await page.waitForTimeout(500);
    const url3 = page.url();
    const body3 = await page.innerText('body');
    const pass3 = url3.endsWith('/charges') && body3.includes('Charges & Fee Profile Workspace');
    ...
    const pass6 = refreshUrl === url3 && refreshBody.includes('Charges & Fee Profile Workspace');
    ...
    const pass7 = segUrl === 'http://localhost:5173/card-programmes/1/segments' && segBody.includes('Customer Segment Eligibility');
